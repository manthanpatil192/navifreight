  ...
  const horizonsData = [
    {
      ...
      congestionDays: destObj.avgWaitDays + (terminalMetrics?.weatherDelayDays || 3.5), // full exposure to anchorage spikes
                  ...
                  {h.id === 1 ? (
                    <div className="mt-2.5 p-2 rounded-lg bg-rose-50/80 border border-rose-200 text-[11px] text-rose-950 space-y-1">
                      <div className="flex items-center space-x-1 font-bold text-rose-800">
                        ...
                        <span>Terminal Risk Advisory:</span>
                      </div>
                      <p className="text-[10.5px] leading-tight text-slate-600">
                        100% exposed to {originObj.name.split('(')[0]} & {destObj.name.split('(')[0]} weather halts (+{(terminalMetrics?.weatherDelayDays || 3.5).toFixed(1)}d wait) and Baltic spot spikes.
                      </p>
                      <div className="text-[10px] font-bold text-rose-700 bg-white/80 rounded px-1.5 py-0.5 border border-rose-200">
                        Directive: AVOID SPOT FIXTURE. Wait till {terminalMetrics?.recommendedWaitDate || 'Sep 15, 2026'} or lock quarterly COA.
                      </div>
                    </div>
                  ) : h.id === 3 ? (
